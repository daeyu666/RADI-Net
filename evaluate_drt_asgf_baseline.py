"""
DRT ResBlock baseline + ASGF block 评估脚本。

示例：
python evaluate_drt_asgf_baseline.py --dataset PaviaU --msi_mode srf --srf_band_set wv2_visible6 --resume checkpoints/baselines/drt_asgf_baseline_PaviaU_wv2_visible6_x4/best.pth
"""
...
def evaluate_and_save(model, loader, device, cfg, save_prediction: bool = True) -> Dict[str, float]:
    ...
    for idx, batch in enumerate(loader):
        ...
        if save_prediction:
            save_path = os.path.join(save_dir, f"drt_asgf_baseline_sample{idx}.mat")
            save_mat(
                save_path,
                {
...
def main():
    ...
    print("=" * 80)
    print("DRT ASGF Baseline Evaluation")
    print("=" * 80)
