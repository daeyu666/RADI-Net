"""
RADI-Net no-ASGF 消融训练脚本。

该脚本复用 train_radi.py 的训练流程，只把模型从 RADINet 换成 RADINetNoASGF。
用于验证 SpectralContextASGF 是否有效。

示例：
python train_radi_no_asgf.py --dataset PaviaU --msi_mode srf --srf_band_set wv2_visible6 --epochs 300 --batch_size 4
"""
...
def main():
    ...
    write_log(log_path, "Model variant: RADINetNoASGF")
