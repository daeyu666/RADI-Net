"""
DRT baseline + all sparse self-attention blocks 评估脚本。

示例：
python evaluate_drt_sparse_all_baseline.py --dataset PaviaU --msi_mode srf --srf_band_set wv2_visible6 --resume checkpoints/baselines/drt_sparse_all_baseline_PaviaU_wv2_visible6_x4/best.pth
"""
...
def evaluate_and_save(model, loader, device, cfg, save_prediction: bool = True) -> Dict[str, float]:
    ...
    for idx, batch in enumerate(loader):
        ...
        if save_prediction:
            save_path = os.path.join(save_dir, f"drt_sparse_all_baseline_sample{idx}.mat")
            save_mat(
                save_path,
                {
...
def main():
    ...
    print("=" * 80)
    print("DRT Sparse All Baseline Evaluation")
    print("=" * 80)
    ...
    with open(metrics_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Sparse positions: {SPARSE_POSITIONS}\n")
        for key, value in metrics.items():
