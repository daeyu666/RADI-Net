"""
DRT ResBlock baseline + ASGF block 训练脚本。

该模型基于 PlainDRTBaseline，只把融合主干中的部分 ResBlock 替换为
HSIContextASGFBlock，默认替换第 2、4、6 个融合 ResBlock。

示例：
python train_drt_asgf_baseline.py --dataset PaviaU --msi_mode srf --srf_band_set wv2_visible6 --epochs 300 --batch_size 4
"""
...
from models.plain_drt_baseline import DRTASGFBaseline
...
def get_run_name(cfg) -> str:
    ...
    return f"drt_asgf_baseline_{cfg.dataset}_{msi_tag}_x{cfg.scale_ratio}"
...
def main():
    ...
    write_log(log_path, "Model variant: DRTASGFBaseline")
    write_log(log_path, "ASGF positions: (1, 3, 5)")
